    ...
    sources: [
      ...
      },
      {
        ...
        config: {
          ...
          query:
            '{ repository(owner: "djheru", name: "croniq") { stargazerCount forkCount issues(states: OPEN) { totalCount } pullRequests(states: OPEN) { totalCount } defaultBranchRef { target { ... on Commit { history(first: 5) { nodes { message committedDate author { name } } } } } } } }',
          headers: {
